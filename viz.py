...
class BoxPlotViz(NVD3Viz):
    ...
    def to_series(self, df, df_o, classed='', title_suffix=''):
        label_sep = " - "
        chart_data = []
        ...
        for index_value, row in zip(df.index, df.to_dict(orient="records")):
            ...
            if isinstance(index_value, tuple):
                for i in range(0, len(index_value)):
                    df_tmp = df_tmp[df_tmp[self.form_data.get("groupby")[i]]==index_value[i]]

                index_value = label_sep.join(index_value)
                
            else:
                df_tmp = df_tmp[df_tmp[self.form_data.get("groupby")[0]]==index_value]
            ...
            for label, box in boxes.items():
                ...
                chart_data.append({
                    "label": chart_label,
                    "values": box,
                    "datas": df_tmp[label].values.tolist()  # 留着元数据
                })
        return chart_data
    ...
    def get_data(self, df):
        ...
        df_res = df.groupby(form_data.get('groupby')).agg(aggregate)
        chart_res = self.to_series(df_res, df)
        return chart_res
